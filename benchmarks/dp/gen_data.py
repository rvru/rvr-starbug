...
coeffs = np.random.randint(-10, 10, size=n_samples, dtype=np.int32)
...
emit("len", np.array(n_samples, dtype=np.uint64))
# emit("n_taps", np.array(n_taps, dtype=np.uint64))
emit("input_1", input_signal, '32')
emit("input_2", coeffs, '32')
emit("output", output, '32')